 ...
 */
var processDelta = function(err, items, end, cb) {
  if(err) {
    return cb(err);
  }
  var files = [];
 ...
 */
var retrievePageOfChanges = function(drive, auth, options, request, cb) {
  if(!request) {
  ...
  request.execute(function(err, resp) {
    ...
    if (nextPageToken) {
      ...
      processDelta(null, results, false, cb);
      retrievePageOfChanges(drive, auth, options, request, results, cb);
    } else {
      processDelta(null, results, true, cb);
    }
  });
};
...
module.exports = function(refreshToken, startChangeId, config, cb) {
  refreshAccessToken(refreshToken, config, function(err, tokens) {
    ...
    getApi(tokens, config, function(drive, auth) {
      ...
      retrievePageOfChanges(drive, auth, options, false, cb);
    });
  });
};
